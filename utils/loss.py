...
class BinaryDiceLoss(nn.Module):
    ...
    def forward(self, input, targets):
        ...
        # Smoothing variable
        smooth = 1
        # Reshape width and height to same dimension
...
class ContrastiveLoss(nn.Module):
    """Contrastive loss to encourage anomaly and normal tokens to learn different features"""
    def __init__(self, temperature=0.1, margin=0.5):
        super().__init__()
        self.temperature = temperature
        self.margin = margin

    def forward(self, anomaly_features, normal_features):
        similarity = F.cosine_similarity(anomaly_features, normal_features, dim=1)
