...
class SegFormerBinary(nn.Module):
    def __init__(self, variant="b2", model_name="", cache_dir=None):
        super().__init__()
        try:
            from transformers import SegformerForSemanticSegmentation
        except ImportError as exc:
            raise ImportError("SegFormer baseline requires transformers. Install requirements.txt first.") from exc

        model_id = model_name or MODEL_IDS[variant]
...
def parse_args():
    parser = argparse.ArgumentParser("Train SegFormer baseline for road waterlogging segmentation")
    ...
    parser.add_argument("--save_path", type=str, default="experiments/segformer_b2")
    ...
    parser.add_argument("--variant", type=str, default="b2", choices=sorted(MODEL_IDS.keys()))
