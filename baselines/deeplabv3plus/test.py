...
def test(args):
    ...
    backbone = args.backbone or config.get("backbone", "resnet50")
    ...
    model = DeepLabV3PlusBinary(backbone=backbone, pretrained_backbone=pretrained_backbone).to(device)
...
def parse_args():
    parser = argparse.ArgumentParser("Test DeepLabV3+ baseline for road waterlogging segmentation")
    ...
    parser.add_argument("--checkpoint_path", type=str, default="experiments/deeplabv3plus/checkpoints/epoch_50.pth")
    parser.add_argument("--save_path", type=str, default="experiments/deeplabv3plus/results")
    parser.add_argument("--image_size", type=int, default=0, help="0 means use checkpoint config")
    parser.add_argument("--backbone", type=str, default="", choices=["", "resnet50", "resnet101"])
