...
SUMMARY_NAME="${SUMMARY_NAME:-adjacent_high_layer_metrics_summary.csv}"

echo "Running DINOv3 ViT-L adjacent high-layer spatial-attention ablation"
echo "Device: ${DEVICE}"
...
for IDX in "${!GROUP_NAMES[@]}"; do
  ...
  echo
  echo "============================================================"
  echo "${GROUP_NAME}: features_list=${FEATURES}, anchors=${NUM_ANCHORS}"
...
echo
echo "Adjacent high-layer spatial-attention ablation completed."
