...
SUMMARY_NAME="${SUMMARY_NAME:-fixed_adapter_spatial_groups_metrics_summary.csv}"
...
echo "Running new-server DINOv2 ViT-L fixed-adapter spatial-attention group experiments"
echo "Device: ${DEVICE}"
echo "Backbone: dinov2:${BACKBONE_NAME}"
echo "Adapter layers: ${ADAPTER_IDS}"
echo "Spatial groups: ${SPATIAL_GROUPS[*]}"
...
for FEATURES in "${SPATIAL_GROUPS[@]}"; do
  FEATURES_TAG="${FEATURES// /_}"
  GROUP_NAME="adapter_3_6_9_12_15_18_21_24_spatial_${FEATURES_TAG}"
  ...
  echo
  echo "${GROUP_NAME}: testing final checkpoint"
...
echo
echo "Completed fixed-adapter spatial-attention group experiments."
